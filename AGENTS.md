...
`loadEnv` picks `.env.production` only when `NODE_ENV=production`, else `.env.development`. `pnpm start` does **not** set `NODE_ENV=production`; PM2 does. Slash register (`pnpm run deploy`) is not part of bot boot: always `.env.production`, always global, then an empty guild PUT to `GUILD_ID` so leftover guild commands disappear. Runtime loads `dist/commands/**/*.js`; deploy discovers `src/**/*.ts`. Keep both paths working.
